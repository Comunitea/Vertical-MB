...
products do not require units for validation'),
        ...
        'hidde_variant_page': fields.function(_get_hidde_variant_page,
                                              ...
                                              method=True,
                                              readonly=True),
        'doorstep_price': fields.float("Doorstep Price", digits=(5, 2)),
        'nook_price': fields.float('Nook Price', digits=(5, 2)),
        'purchase_price': fields.float('Purchase Price', digits=(5, 2)),
    }
    _defaults = {
...
class product_product(models.Model):
    ...
    def get_uos_logistic_unit(self, uos_id):
        if uos_id == self.log_base_id.id:
            ...
            raise except_orm(_('Error'), _('The product [%s] unit of '
                                           'measure %s '
                                           'is \
                             not related with any logistic \
                             unit' % self.default_code,self.uom_id.name))
